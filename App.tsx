...
import { CreatorStep, Message, Language, NovelSettings, ConsistencyMemory, DEFAULT_STEP_MODELS } from './types';
import { sendMessageToGemini, updateConfig, testApiConnection, generateConsistencySummary } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const handleSendMessage = async (overrideText?: string) => {
    ...
    const userMsg: Message = {
      id: nanoid(), role: 'user', content: textToSend, timestamp: Date.now(), step: currentStep
    };
    ...
    try {
      const settingsContext = `[Target: ${novelSettings.targetAudience}, Words: ${novelSettings.totalWordCount}, Ch: ${novelSettings.chapterWordCount}]`;
      const systemContextLang = language === 'zh' 
        ? `\n\n[System: 当前阶段 ${currentStep}。必须全中文。${settingsContext}]`
      ...
      const aiMsg: Message = {
        id: nanoid(), role: 'model', content: responseText, timestamp: Date.now(), step: currentStep
      };
      ...
      if (responseText.length > 200 || currentStep >= 7) {
        generateConsistencySummary(responseText, memoryBank).then(updatedMemory => {
          ...
          console.log("Memory Bank Updated for consistency.");
        });
      }
      ...
      setMessages(prev => [...prev, {
        id: nanoid(), role: 'system', timestamp: Date.now(),
        content: language === 'zh' ? "遇到了网络错误。" : "Network error."
      }]);
    } finally {
      setIsLoading(false);
      setActiveModel('');
  ...
  const handleSaveSettings = () => {
    ...
    setMessages(prev => [...prev, { id: nanoid(), role: 'system', content: language === 'zh' ? "配置已更新。" : "Config updated.", timestamp: Date.now() }]);
  };

  const toggleLanguage = () => setLanguage(prev => prev === 'en' ? 'zh' : 'en');
  const openEditModal = (key: keyof NovelSettings) => setEditValueModal({ isOpen: true, key, value: novelSettings[key] });
  const handleSaveValue = () => {
    if (editValueModal.key) {
      setNovelSettings(prev => ({ ...prev, [editValueModal.key!]: editValueModal.value }));
      setEditValueModal({ isOpen: false, key: null, value: '' });
      setMessages(prev => [...prev, { id: nanoid(), role: 'system', content: `[Update] ${t[editValueModal.key!]} -> ${editValueModal.value}`, timestamp: Date.now() }]);
    }
  };
      ...
      <div className="flex-1 flex flex-col h-full relative">
        <header className="h-16 absolute top-0 left-0 right-0 bg-white/80 backdrop-blur-md border-b border-white/20 shadow-sm flex items-center justify-between px-4 z-30 shrink-0">
          <div className="flex items-center gap-3">
            <button onClick={() => setIsSidebarOpen(true)} className="lg:hidden p-2 text-slate-500 hover:bg-slate-100 rounded-full transition-colors">
          ...
          <div className="max-w-3xl mx-auto flex items-end gap-3">
            <div className="relative flex-1 group">
              <textarea
                ...
                className="w-full relative bg-white border border-slate-200 rounded-3xl pl-6 pr-14 py-5 text-base text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary shadow-lg shadow-slate-200/50 min-h-[80px] transition-all"
                rows={1}
              />
              <button
                onClick={() => handleSendMessage()}
                disabled={isLoading || !input.trim()}
                className={`absolute right-3 bottom-3 p-3 rounded-2xl transition-all ${input.trim() ? 'bg-primary text-white hover:scale-105 shadow-primary/30' : 'bg-slate-100 text-slate-300'}`}
              >
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl p-8 transform transition-all">
            <div className="flex justify-between items-center mb-8"><h3 className="text-xl font-bold text-slate-800">{t.settings}</h3><button onClick={() => setShowSettings(false)} className="p-2 hover:bg-slate-100 rounded-full transition-colors"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5 text-slate-400"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button></div>
      ...
      {editValueModal.isOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-sm shadow-2xl p-8 transform transition-all">
            <h3 className="text-lg font-bold text-slate-800 mb-4">{t.editTitle}</h3>
            <div className="mb-6"><input type="text" value={editValueModal.value} onChange={(e) => setEditValueModal(prev => ({ ...prev, value: e.target.value }))} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-base focus:ring-2 focus:ring-primary/20" autoFocus /></div>
            <div className="flex gap-3"><button onClick={() => setEditValueModal({ isOpen: false, key: null, value: '' })} className="flex-1 px-4 py-2 border border-slate-200 rounded-xl text-slate-600 font-bold">取消</button><button onClick={handleSaveValue} className="flex-1 px-4 py-2 bg-primary text-white rounded-xl font-bold hover:bg-indigo-600">保存</button></div>
          </div>
        </div>
      )}
    </div>
